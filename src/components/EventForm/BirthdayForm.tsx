...
import { Birthday } from "@/src/core/interfaces/Events";
...
export const BirthdayForm: React.FC<BirthdayFormProps> = ({
	...
	const [formData, setFormData] = React.useState<Partial<Birthday>>({
		...event,
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const completeEvent: Birthday = {
			...
			nextBirthday: updatedFormData.nextBirthday || "",
			age: updatedFormData.age,
			animal: updatedFormData.animal,
			zodiac: updatedFormData.zodiac,
			isHidden:
	...
	return (
		<form className="yg-event-form" onSubmit={handleSubmit}>
			<h3 className="yg-event-form-title">
				...
				{t(`view.eventManager.birthday.name`)}
			</h3>
			...
			{isEditing && (
				<>
					...
					<div className="form-group read-only">
						...
						{renderReadOnlyValue(formData.nextBirthday)}
					</div>
					<div className="form-group read-only">
						<label>{t("view.eventManager.birthday.animal")}</label>
						{renderReadOnlyValue(formData.animal)}
					</div>
					<div className="form-group read-only">
						<label>{t("view.eventManager.birthday.zodiac")}</label>
						{renderReadOnlyValue(formData.zodiac)}
					</div>
				</>
			)}
			...
			>
				...
				<div className="form-group">
					...
					<input
						...
						onChange={handleChange}
						placeholder={EVENT_TYPE_DEFAULT.birthday.emoji}
					/>
				</div>
				...
				<div className="form-group">
					...
					<ColorSelector
						...
						defaultColor={EVENT_TYPE_DEFAULT.birthday.color}
